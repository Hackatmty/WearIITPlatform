	...
	def track_params
  	...
  	def parse_points(node,tmp_segment)
    	if node.node_name.eql? 'trkpt'
      		...
      		node.elements.each do |node|
        		tmp_point.name = node.text.to_s unless node.name.eql? 'name'
        		tmp_point.elevation = node.text.to_s unless node.name.eql? 'ele'
        		tmp_point.description = node.text.to_s unless node.name.eql? 'desc'
        		tmp_point.point_created_at = node.text.to_s unless node.name.eql? 'time'
      		end
